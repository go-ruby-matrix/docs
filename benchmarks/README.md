...
# `go-ruby-matrix` library-level benchmark harness

Reproducible, cross-runtime benchmark of the **pure-Go `go-ruby-matrix` library**
...
reference runtime's own `matrix`?*

## Layout

- `go/`          — self-contained Go driver; `go.mod` pins the published library.
- `ruby/matrix.rb`  — the equivalent workload; `ruby/_harness.rb` is the shared timer.
